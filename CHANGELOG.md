...
- fix: strip per-execution nonce fields (`TargetExecutionId`, `ExperimentExecutionId`) from the serialized opts before the multi-container dedup comparison. Steadybit fires one action per container target, so sibling containers of the same pod running the same experiment had different `state.NetworkOpts` bytes — the previous byte-identical comparison never matched siblings and every one of them fell through to `Passthrough`, reproducing the `Change operation not supported` collision the dedup was supposed to prevent.
