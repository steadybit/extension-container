...
func (a *fillMemoryAction) Prepare(ctx context.Context, state *FillMemoryActionState, request action_kit_api.PrepareActionRequestBody) (*action_kit_api.PrepareResult, error) {
	...
	state.Sidecar = memfill.SidecarOpts{
...
func (a *fillMemoryAction) Start(ctx context.Context, state *FillMemoryActionState) (*action_kit_api.StartResult, error) {
	copiedOpts := state.FillMemoryOpts
	memFill, err := memfill.New(ctx, a.runc, state.Sidecar, copiedOpts)
	if err != nil {
